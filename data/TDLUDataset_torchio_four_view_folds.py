...
class TdludatasetTorchioFourViewFolds(Dataset):
    ...
    def __init__(
        ...
        for sid in self.subjects:
            ...
            idx = 0 if raw == 0 else bin_value_quantile(raw, self.thresholds) + 1
            bins.append(idx)
        counts = Counter(bins)
        ...
        if purpose in ['train', 'validation', 'test']:
    ...
    def __getitem__(self, idx):
        ...
        raw = data['target']
        idx = 0 if raw == 0 else bin_value_quantile(raw, self.thresholds) + 1
        label = F.one_hot(torch.tensor(idx), num_classes=self.num_bins).float()
