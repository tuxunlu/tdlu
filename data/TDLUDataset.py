...
class TDLUDataset(Dataset):
    def __init__(
        ...
        # valid_values = numeric_target[(numeric_target.notna())]
        ...
        for img_file in all_image_files:
            ...
            if self.label:
                bin_idx = tval
                print(f"Using raw target value {tval} for label {self.label} in {img_file}")
            else:
    ...
    def __configure_augmentation(self):
        ...
        if self.purpose == 'train' and self.use_augmentation:
            return transforms.Compose([
                ...
                ToTensor16RGB(),
                transforms.RandomApply(spatial, p=self.aug_prob),
...
if __name__ == "__main__":
    # Example usage
    train_dataset = TDLUDataset(
        image_dir="/fs/nexus-scratch/tuxunlu/git/tdlu/dataset/WUSTL_png_nomarker_16",
        csv_path="/fs/nexus-scratch/tuxunlu/git/tdlu/dataset/umd_annot_md_TDLU_y2025m04d11.csv",
        num_bins=4,
        target="tdlu_density",
        purpose="train",
        use_augmentation=True,
