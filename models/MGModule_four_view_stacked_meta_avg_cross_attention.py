...
class MgmoduleFourViewStackedMetaAvgCrossAttention(nn.Module):
    """
    ...
    def forward(self, views: torch.Tensor, meta: torch.Tensor):
        """
        views: Tensor of shape [B, C, H, W], where C=4 views.
        meta:  Tensor [B, 4] → the tabular [age, BMI, ancestry]
        Returns:
          logits: [B, num_bins]
...
if __name__ == "__main__":
    model = MgmoduleFourViewStackedMetaAvgCrossAttention(num_bins=2)
    ...
    logits, fused = model(dummy_views, dummy_meta)
