...
class MgmoduleFourViewMeta(nn.Module):
    """
    ...
    def __init__(
        ...
        # per-view FiLM generator: 1→512  (from density scalar → scale and shift)
        self.density_film = nn.Sequential(
            nn.Linear(1, 512*2),    # outputs [γ; β]
            nn.LayerNorm(512*2),
            nn.GELU()
        )
    ...
    def forward(self, views: torch.Tensor, density: torch.Tensor, meta: torch.Tensor):
        ...
        film_feats = []
        for i in range(V):
            gamma, beta = self.density_film(density[:, i:i+1]).chunk(2, -1)
            film_feats.append(gamma * mamm_feats[:, i] + beta)
        film_feats = torch.stack(film_feats, dim=1)      # [B, V, D]
        ...
        view_tokens = film_feats.permute(1,0,2)         # [V,B,D]
